...
/**
 * Uniqueness-gated generation: ensures
 * - givens are consistent with the generated solution
 * - the puzzle has exactly one solution
 *
 * Intended for use as a non-blocking fallback when pack content is unavailable.
 */
export function generateUnique(
  ...
  const maxAttempts = Math.max(1, Math.min(200, Math.floor(opts?.maxAttempts ?? 50)));
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    // Step 2: remove cells until we reach target givens count while staying solvable.
    const targetGivens = givensForDifficulty(difficulty);
    const puzzleArr = solution.slice() as CellValue[];
  ...
  throw new Error(`Failed to generate unique puzzle for difficulty=${difficulty}`);
}
